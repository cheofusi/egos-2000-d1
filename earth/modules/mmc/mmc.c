...
static void dumphex32(char *name, char *base, int len)
{
	...
	INFO("dump %s registers:", name);
	for (i = 0; i < len; i += 4) {
		if (!(i & 0xf))
			INFO("\n0x%x : ", base + i);
		INFO("%x ", *(unsigned int *)(base + i));
	}
	INFO("\n");
}
...
retry:
	...
	if (ret < 0) {
		INFO("%s %d:read region parameter fail, %s \n", __func__, __LINE__,
	...
	if (pregion->header.magic == SDMMC_PARAMETER_MAGIC) {
		...
		if (sum != add_sum) {
			INFO("%s %d:region add sum(%x) is not right(%x), %s \n",
		...
		INFO("%s %d:region magic is not right, %s %x\n", __func__, __LINE__,
...
int mmc_startup(struct mmc *mmc)
{
	...
	// mmcinfo("%u Hz\n", mmc->clock);
	// mmcinfo("%u MB\n", mmc->lba >> 11);
	mmcinfo("%u MB %s %u bit SD card @%uHz", mmc->lba >> 11, spd_name[mmc->speed_mode],
...
retry_card:
	...
	if (err) {
		...
		mmc->has_init = 1;
		mmcinfo("SD/MMC %u init OK!!!", mmc->control_num);
	}
